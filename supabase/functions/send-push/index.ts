...
async function buildVapidJwt(
  ...
  const sig = new Uint8Array(
    await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, cryptoKey, enc.encode(toSign)),
  );
...
async function encryptPayload(
  ...
  const ikm = await hkdf(authSecret, sharedBits, buildLabel("Content-Encoding: auth\0", serverPublicKeyRaw, recipientPublicKeyRaw, 32));
  const cek = await hkdf(salt, ikm, buildLabel("Content-Encoding: aes128gcm\0", serverPublicKeyRaw, recipientPublicKeyRaw, 16));
  const nonce = await hkdf(salt, ikm, buildLabel("Content-Encoding: nonce\0", serverPublicKeyRaw, recipientPublicKeyRaw, 12));
...
function buildLabel(info: string, senderPub: Uint8Array, recipientPub: Uint8Array, length: number): Uint8Array {
  const enc = new TextEncoder();
  const infoBytes = enc.encode(info);
  const out = new Uint8Array(infoBytes.length + 1 + 2 + senderPub.length + 2 + recipientPub.length + 2);
  let off = 0;
  out.set(infoBytes, off); off += infoBytes.length;
  out[off++] = 0; // null terminator already in info string
  // key_length as uint16
  out[off++] = 0; out[off++] = senderPub.length;
  out.set(senderPub, off); off += senderPub.length;
  out[off++] = 0; out[off++] = recipientPub.length;
  out.set(recipientPub, off); off += recipientPub.length;
  out[off++] = 0; out[off] = length;
  return out.slice(0, out.length - 1 + 1);
}

async function hkdf(salt: Uint8Array, ikm: Uint8Array, info: Uint8Array): Promise<Uint8Array> {
...
serve(async (req) => {
  ...
  try {
    ...
    if (subsError || !subs?.length) {
      return new Response(
        JSON.stringify({ sent: 0, reason: subsError?.message ?? "no subscriptions" }),
    ...
    const results = await Promise.allSettled(
      subs.map((sub) =>
        sendPush(sub.endpoint, sub.p256dh, sub.auth, payload, VAPID_PUBLIC_KEY!, VAPID_PRIVATE_KEY!, VAPID_SUBJECT),
      ),
    );
