...
// Adjust this import to match your actual ThemeContext path
// It should expose a `theme` string like 'dark' or 'light', or CSS variables via :root
    ...
    <div className="notifications-tab">
      ...
      >
        <Toggle
          icon={<MessageCircle size={15} />}
          label="Messages"
          description="Email alerts for new direct messages"
          checked={prefs?.email_messages ?? true}
          onChange={(v) => updatePref("email_messages", v)}
        />
        <Toggle
          icon={<Tag size={15} />}
          label="Offers & deals"
          description="When sellers send you special offers"
    ...
    <div className="push-banner push-banner--default">
      <div className="push-banner__left">
        ...
        <div>
          ...
          <p className="push-banner__sub">
            Get alerts without being on the app
          </p>
        </div>
      </div>
      <button className="push-banner__btn" onClick={onRequest}>
        Enable
      </button>
    </div>
  );
...
function Toggle({ icon, label, description, checked, onChange, disabled }) {
  return (
    <label
      className={`notif-toggle${disabled ? " notif-toggle--disabled" : ""}`}
    >
...
// ─── Styles (theme-adaptive via CSS variables) ────────────────────────────────
// These variables read from your ThemeContext / :root declarations.
// They fall back to sensible dark defaults if not set.
...
const STYLES = `
  ...
  .notif-switch--on .notif-switch__thumb {
